...
impl Inventory
{
    ...
    pub fn max(&self, other : &Inventory) -> Inventory
    {
        Inventory
        {
...
impl Blueprint
{
    ...
    pub fn parse(s : &str) -> Option<Blueprint>
    {
        ...
        for (resource, mut words) in Resource::enumerate().zip(s.split('.').map(|s| s.split_whitespace()))
        {
...
impl Resource
{
    pub fn enumerate() -> impl Iterator<Item = Resource>
    {
        [Resource::Ore,
         Resource::Clay,
         Resource::Obsidian,
