...
pub fn interpret(memory : &mut [i64], snd : SyncSender<i64>, rcv : Receiver<i64>)
{
    let mut ip = 0;
    loop
    {
        ...
        match opcode
        {
            ...
            03 => input(                                   &modes, &mut ip, memory, &rcv),
            04 => output(                                  &modes, &mut ip, memory, &snd),
            ...
            99 => break,
            _  => { eprintln!("invalid opcode: {}, terminating!", opcode); break }
        };
    }
}
...
fn input(modes : &Modes, ip : &mut usize, memory : &mut [i64], rcv : &Receiver<i64>)
{
    match rcv.recv()
    {
        Ok(x)  => *index_modal(modes[0], *ip+1, memory) = x,
        Err(_) => { println!("Fs in chat"); }
    }
    *ip += 2
}

fn output(modes : &Modes, ip : &mut usize, memory : &mut [i64], snd : &SyncSender<i64>)
{
    snd.send(*index_modal(modes[0], *ip+1, memory));
    *ip += 2
}
